...
namespace Chess_With_Forms
{
    public partial class Form1 : Form
    {
        ...
        {
            (InactiveTimer.Text, ActiveTimer.Text) = (ActiveTimer.Text, InactiveTimer.Text);
        }
        private void OnTimeEvent(object sender, System.Timers.ElapsedEventArgs e)
        {
            Invoke(new Action(() =>
            {
                int s = int.Parse(ActiveTimer.Text[3].ToString() + ActiveTimer.Text[4].ToString());
                int m = int.Parse(ActiveTimer.Text[0].ToString() + ActiveTimer.Text[1].ToString());
                s--;
                if (s < 0)
                {
                    s = 59;
                ...
                if (piece.Left / 60 != 7)
                {
                    if (board[piece.Top / 60 - 1, piece.Left / 60 + 1] != "_")
                    {
                        if (board[piece.Top / 60 - 1, piece.Left / 60 + 1][0] != piece.Name[0])
                        {
                            ...
                            {
                                if (!board[piece.Top / 60 - 1, piece.Left / 60 + 1].Contains('F'))
                                {
                                    board[piece.Top / 60 - 1, piece.Left / 60 + 1] += "F";
                                }
